...
const ArgParseState = enum { Init, ReadEnpoint, ReadMessage };
...
pub fn main() !void {
    ...
    var endpoint = try allocator.dupe(u8, "/tmp/com_example_echo.sock");
    defer allocator.free(endpoint);
    var messageToSend = try allocator.dupe(u8, "Hi, there!");
    defer allocator.free(messageToSend);
        ...
        switch (state) {
            ArgParseState.Init => {
                if (std.mem.eql(u8, arg, "-e") or std.mem.eql(u8, arg, "--endpoint")) {
                    ...
                    state = ArgParseState.ReadMessage;
                } else {
                    std.debug.print("usage: echo-client [-e <endpoint>] [-m <message>]\n", .{});
                    return;
                }
            },
            ...
            ArgParseState.ReadMessage => {
                allocator.free(messageToSend);
                messageToSend = try allocator.dupe(u8, arg);
                state = ArgParseState.Init;
            },
        }
    }
    ...
    try send_message(connection, messageToSend);
